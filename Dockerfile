...
ARG APP_PATH=/icalia-actions/run-ci-compose
...
ARG APP_PATH=/icalia-actions/run-ci-compose
COPY --from=builder --chown=node:node ${APP_PATH} /icalia-actions/run-ci-compose
WORKDIR /icalia-actions/run-ci-compose
